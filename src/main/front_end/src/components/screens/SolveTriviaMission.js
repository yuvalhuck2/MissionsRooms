...
class SolveTriviaMission extends Component{
    ...
    renderQuestionAnswers({answers, correctAnswer}){
        ...
        let answerList = (
            <FlatList data={answers} renderItem={renderItem} keyExtractor={(item) => item}/>
        )


        return(
            <RadioButtonGroup style={{flex: 1, padding: 50}} onValueChange={()=>{}} value={correctAnswer}>
                {answerList}
            </RadioButtonGroup>
        )
    }
        ...
        let renderItem = ({item}) => {
            return (
                <ListAccordion
                    ...
                    id={item.id}
                >
                    {this.renderQuestionAnswers({answers: item.answers, correctAnswer: item.correctAnswer})}
                </ListAccordion>
            )
        }
        return(
