...
import { Student, Supervisor, Teacher } from "../../actions/OpCodeTypes";
...
const { enter_search, name, role, exit } = WatchProfileStrings;

const { student, teacher, supervisor } = RolesStrings;

const { sure, yes, no } = DeleteUserString;
const {
  edit_details,
  delete_user,
  enter_last_name,
...
class ManageUsersForm extends Component {
  ...
  onEditButtonPress() {
    ...
    return this.props.editUserDetails({
      apiKey,
      firstName,
      lastName,
      alias: profile,
  ...
  onDeleteButtonPress() {
    ...
    return Alert.alert(sure, "", [
      {
        text: yes,
        onPress: () => this.props.deleteUser({ apiKey, alias: profile }),
      },
      { text: no, onPress: () => {} },
    ]);
  }
  ...
  renderButtons() {
    ...
    return loading ? this.renderSpinner() : this.createDialogActionsList();
  }
    ...
    if (currUser !== undefined) {
      let lst = [
        ...
        <Dialog.Actions key="delete">
          <Button styles={styles.button} onPress={this.onDeleteButtonPress}>
            {delete_user}
          </Button>
        </Dialog.Actions>,
      ];
      if (currUser.userType === "Student") {
        lst.push(
          <Dialog.Actions key="moveToAnotherGroup">
            <Button styles={styles.button} onPress={this.passToTransferGroup}>
              {transfer}
            </Button>
          </Dialog.Actions>
        );
      ...
      if (currUser.userType === "Teacher" && !currUser.hasClassroom) {
        lst.push(
          <Dialog.Actions key="moveToAnotherClass">
            <Button
              styles={styles.button}
          ...
          <Dialog.Content>
            <DataTable.Row>
              ...
              <Switch
                ...
                onValueChange={this.setEnableFirstName}
              />
            </DataTable.Row>
            <DataTable.Row>
              <DataTable.Cell>{enable_edit_last_name}</DataTable.Cell>
              <Switch
                ...
                onValueChange={this.setEnableLastName}
              />
            </DataTable.Row>
            {enableFirstName ? (
              <TextInput
