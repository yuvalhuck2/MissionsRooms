...
import {
  ...
  handleBack,
  suggestionChanged,
} from "../../actions/WatchSuggestionsActions";
...
const { delete_suggestion, watch_suggestions, exit } = WatchSuggestionsStrings;
    ...
    return (
      <SafeAreaView style={styles.container}>
        <KeyboardAwareScrollView>
          ...
          <Portal>
            <Dialog
              visible={Object.keys(suggestion).length !== 0}
              onDismiss={() => this.onSuggestionChanged({})}
            >
              <Dialog.Content>
              ...
              <Dialog.Actions>
                <Button
                  styles={styles.button}
                  onPress={() => this.onSuggestionChanged({})}
                >
                  {exit}
                </Button>
              </Dialog.Actions>
